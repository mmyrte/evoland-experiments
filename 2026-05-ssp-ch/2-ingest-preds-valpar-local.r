...
local_pred_specs <- list(
  ...
  slope = list(
    unit = "deg",
    pretty_name = "Slope in degrees, likely using raster::terrain",
    orig_format = "100m raster",
    description = "Originally swissAlti3D? Came via valpar.ch, and thence via Speedmind?",
    sources = list(
      list(
...
local_pred_sources <-
  local_pred_specs |>
  evoland:::pluck_wildcard(NA, "sources") |>
...
for (pred_name in names(local_pred_specs)) {
  db$add_predictor(
    pred_data_raw = pred_data_long[
      layer == pred_name,
      .(id_coord, id_period, value)
    ],
    name = pred_name,
    fill_value = NA, # for none of these is it sensible to substitute 0
    unit = local_pred_specs[[pred_name]][["unit"]],
    pretty_name = local_pred_specs[[pred_name]][["pretty_name"]],
    orig_format = local_pred_specs[[pred_name]][["orig_format"]],
    description = local_pred_specs[[pred_name]][["description"]],
    sources = local_pred_specs[[pred_name]][["sources"]] |> rbindlist()
  )
}
