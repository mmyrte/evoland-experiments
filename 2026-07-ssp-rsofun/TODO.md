...
- [x] `010-forcing-climate.r` — CH2025 daily → derived forcings (vpd, ccov via
      Hargreaves, rain/snow split, patm; SPLASH does PPFD internally).
- [x] `010-forcing-soil-download.r` — fetch SSPM mean GeoTIFFs (Zenodo 7821650).
- [x] `010-landcover-fapar.r` — parse WASIM `[landuse_table]` → daily
      fAPAR/albedo/rootdepth per class.
- [x] `011-forcing-soil-whc.r` — SSPM (PTF / `soil_hydro`) → per-layer AWC profile.
      Consumes the download above, hence the separate stage number.
...
- [ ] **`001-setup-db.r`** — attach/reuse `ssp-ch.evolanddb` baseline; register the
      rsofun run(s) (`id_run`).
- [ ] **`020-run-rsofun.r`** — per-pixel P-model (+ BiomeE) over decades; warm-start
      each decade from the previous end state; chunked for parallelism.
- [ ] **`030-aggregate-indicators.r`** — daily → decadal α (AET/PET), `wscal`, soil
      moisture, GPP (+ BiomeE NPP/biomass/LAI) → `db$add_predictor`.
- [ ] **`040-couple-decadal-loop.r`** — feed rsofun predictors to the transition
...
- [ ] **AltDep elevation phenology.** `010-landcover-fapar.r` parses `AltDep` but the
      ...
      gradient). (`010-landcover-fapar.r:28`; README §8.1)
- [ ] **Soil download pattern.** Align `010-forcing-soil-download.r` with the shared
      `download_and_verify` ingestion pattern used elsewhere.
      (`010-forcing-soil-download.r:17`)
- [ ] **VPD dewpoint bias.** Quantify the Alpine `dewpoint ≈ tmin` dry bias against
      MeteoSwiss RH stations. (`010-forcing-climate.r:64`; README §8.2)
